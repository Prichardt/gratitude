...
const endpoint = computed(() =>
    `/internal-api/gratitude/${props.gratitudeNumber}/${props.pointType === 'earned' ? 'earned' : 'bonus'}/${props.point.id}`
);
            ...
            <div class="bg-card w-full max-w-md rounded-xl shadow-2xl border border-destructive/20 overflow-hidden">
                ...
                <div class="bg-destructive text-white px-6 py-4 flex justify-between items-center">
                    <h2 class="text-sm font-semibold tracking-wide flex items-center gap-2">
                        <Trash2 class="w-4 h-4 opacity-80" /> Delete {{ pointLabel }}
                    </h2>
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div class="flex items-start gap-3 bg-red-50/80 dark:bg-red-950/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
                        ...
                        <p class="text-sm text-red-800 dark:text-red-300 font-medium">
                            This will <strong>permanently delete</strong> this {{ pointLabel.toLowerCase() }} entry and cannot be undone. The account balance will be updated.
                        </p>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div class="bg-red-50/70 dark:bg-red-950/20 rounded-lg p-3 border border-red-200/50 dark:border-red-800/50">
                            ...
                            <p class="text-lg font-bold text-red-700 dark:text-red-300 mt-0.5">{{ formatNum(point.points) }} pts</p>
                        </div>
                    </div>
                    <div v-if="point.description || point.category" class="bg-muted/30 rounded-lg p-3 border border-border/40">
                        <p class="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">Description</p>
                        <p class="text-sm mt-0.5">{{ point.description || point.category }}</p>
                    </div>
